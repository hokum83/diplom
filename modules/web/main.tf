...
resource "yandex_alb_backend_group" "backend-group-1" {
  ...
  depends_on = [yandex_compute_instance_group.web-group-1]
  http_backend {
    name             = "diplom-http-backend"
    weight           = 1
    port             = 80
    target_group_ids = [yandex_compute_instance_group.web-group-1.application_load_balancer.0.target_group_id]
...
resource "yandex_resourcemanager_folder_iam_member" "editor" {
  ...
  depends_on = [yandex_iam_service_account.service-account-1]
}

resource "yandex_compute_instance_group" "web-group-1" {
  name                = "web-group"
  folder_id           = var.folder_id
...
data "yandex_compute_instance" "web_instances" {
  depends_on  = [yandex_compute_instance_group.web-group-1]
  count       = yandex_compute_instance_group.web-group-1.scale_policy[0].fixed_scale[0].size
  instance_id = yandex_compute_instance_group.web-group-1.instances.*.instance_id[count.index]
}
